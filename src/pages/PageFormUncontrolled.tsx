...
export const PageFormUncontrolled = () => {
  ...
  const validateForm = async (event: React.FormEvent) => {
    ...
    const formData: UserDataDispatch = {
      ...
      gender: inputMale.current?.checked
    ...
    try {
      ...
      const errors: Record<string, string> = (
        validationErrors as { inner: Array<{ path: string; message: string }> }
      ).inner.reduce((acc: Record<string, string>, error: { path: string; message: string }) => {
    ...
    <main className="page">
      <form onSubmit={(event) => validateForm(event)}>
        ...
        <div className="input-box">
          ...
          {error.userPass && <span className="error">{error.userPass}</span>}
        ...
        <div className="input-box">
          ...
          {error.userPassRepeat && <span className="error">{error.userPassRepeat}</span>}
        ...
        <fieldset>
          ...
          <div>
            <input ref={inputMale} id="userMale" type={'radio'} value="male" name="gender" />
            <label htmlFor="userMale">male</label>
          </div>
          <div>
            <input ref={inputFemale} id="userFemale" type={'radio'} value="female" name="gender" />
        ...
        <div className="input-box">
          ...
          {error.userFile && <span className="error">{error.userFile}</span>}
